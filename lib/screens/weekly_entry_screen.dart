...
class _WeeklyEntryScreenState extends ConsumerState<WeeklyEntryScreen> {
  ...
  void initState() {
    super.initState();
    _weekStartDate = _getWeekStart(DateTime.now());
  }
  ...
  Future<void> _selectWeekStart() async {
    final picked = await showDatePicker(
      context: context,
      initialDate: _weekStartDate ?? _getWeekStart(DateTime.now()),
      firstDate: DateTime(2020),
      lastDate: DateTime(2030),
    );
    if (picked != null) {
      setState(() {
        _weekStartDate = _getWeekStart(picked);
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Weekly Collection Entry'),
                ...
                InkWell(
                  ...
                  child: InputDecorator(
                    decoration: const InputDecoration(
                      ...
                      hintText: 'Select week start date',
                      border: OutlineInputBorder(),
                      prefixIcon: Icon(Icons.calendar_view_week_rounded),
                    ),
                    child: Text(
                      _weekStartDate != null
                          ? DateFormat('MMM dd, yyyy').format(_weekStartDate!)
  ...
  Future<void> _submitEntry() async {
    if (_weekStartDate == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Please select a week start date'),
    ...
    try {
      await service.createDailyCashRecord(
        entryDate: _weekStartDate!,
        regionId: _selectedRegionId!,
        modelId: _selectedModelId!,
        bagId: _selectedBagId!,
        weekStartDate: _weekStartDate!,
        dayOfWeek: _weekStartDate!.weekday % 7,
        netAmountInHand: _parseAmount(_netAmountController.text),
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Weekly entry submitted successfully!'),
